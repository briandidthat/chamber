...
//            console.log(provider.getBalance("0x9D727911B54C455B0071A7B682FcF4Bc444B5596"));
    ...
    static async findBestQuote(sellToken, buyToken, amount) {
        try {
            ...
            const fromTokenAddress = getTokenAddress(sell);
            const toTokenAddress = getTokenAddress(buy);

            const sellAmount = sell === "USDC" ? ethers.utils.parseUnits(amount, "mwei") : ethers.utils.parseEther(amount);

            const [zeroXQoute, oneInchQoute, paraswapQoute] = await Promise.all([
                this.fetchOxQuote(sell, buy, sellAmount),
                this.fetchOneInchQoute(fromTokenAddress, toTokenAddress, sellAmount),
                this.fetchParaSwapQoute(fromTokenAddress, toTokenAddress, sellAmount)]);
            ...
            const sorted = [
                {liquiditySource: LIQUIDITY_SOURCES.ZERO_X, expectedOutput: zeroXAmount, response: zeroXQoute},
                {liquiditySource: LIQUIDITY_SOURCES.ONE_INCH, expectedOutput: oneInchAmount, response: oneInchQoute},
                {liquiditySource: LIQUIDITY_SOURCES.PARASWAP, expectedOutput: paraswapAmount, response: paraswapQoute}
